 ...
 */
const SLUG_INPUT_CAP = 256;
const SLUG_NON_ALNUM = /[^a-z0-9]+/g;

export function generateSlug(name: string): string {
  const capped = name.length > SLUG_INPUT_CAP ? name.slice(0, SLUG_INPUT_CAP) : name;
  const lowered = capped.toLowerCase().replace(SLUG_NON_ALNUM, "-");

  let start = 0;
  let end = lowered.length;
  while (start < end && lowered.charCodeAt(start) === 45) start++;
  while (end > start && lowered.charCodeAt(end - 1) === 45) end--;

  return lowered.slice(start, end).substring(0, 63);
}
