...
const PREVIEW_INPUT_CAP = 8192;
const TAG_PATTERN = /<[^<>]*>?/g;
const STRAY_LT_PATTERN = /</g;
...
export function generatePreviewText(html: string, maxLength: number = 100): string {
  const bounded = html.length > PREVIEW_INPUT_CAP ? html.slice(0, PREVIEW_INPUT_CAP) : html;
  ...
  text = text.replace(STRAY_LT_PATTERN, "");

  // Decode HTML entities. `&amp;` MUST be decoded last so `&amp;lt;` stays `&lt;`
  // and is not collapsed into `<` (CodeQL js/double-escaping).
  const decoded = text
    .replace(/&nbsp;/g, " ")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
...
export function isValidEmail(email: string): boolean {
  ...
  return EMAIL_LOCAL_PATTERN.test(local) && EMAIL_DOMAIN_PATTERN.test(domain);
}
