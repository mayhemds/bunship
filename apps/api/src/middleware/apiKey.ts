...
import { getDatabase, users, eq, and, isNull } from "@bunship/database";
...
export const optionalApiKeyMiddleware = new Elysia({
  ...
  async (context): Promise<{ apiKey: ApiKey | null; organization: { id: string } | null }> => {
...
export function requireScope(scope: string) {
  return new Elysia({ name: `requireScope:${scope}` }).derive(async (context): Promise<void> => {
    const { set } = context;
    const apiKey = (context as any).apiKey as ApiKey | undefined;
  ...
  async (
    ...
    if (authorization?.startsWith("Bearer ")) {
      ...
      try {
        ...
        // Verify the user is still active and not deleted — a valid JWT
        // issued before deactivation must not keep working.
