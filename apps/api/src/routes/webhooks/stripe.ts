...
import { getDatabase, subscriptions, webhookEvents, eq } from "@bunship/database";
  ...
  .post(
    ...
    async ({ request, set }) => {
      try {
        ...
        // Atomic idempotency: insert the event id FIRST. If a concurrent
        // delivery already won the race, the primary-key conflict makes this
        // a no-op and we return early without re-processing.
