...
import { invitations, memberships, users } from "@bunship/database/schema";
...
const orgInvitationRoutes = new Elysia({
  ...
  .post(
    ...
    async ({ organization, user, body, set }) => {
      ...
      // Generate invite URL
      const inviteUrl = `${appConfig.frontendUrl}/invite/${token}`;
 ...
 */
const acceptInvitationRoute = new Elysia({ tags: ["Invitations"] })
  ...
  .post(
    ...
    async ({ params, user, set }) => {
      ...
      // Return an identical 400 for "not found", "expired", "org gone", and
      // "wrong user" so callers can't enumerate which tokens exist or infer
      // whose mailbox was invited.
      const INVALID_TOKEN_MSG = "Invitation is invalid, expired, or not addressed to you";

      if (!invitation) {
        set.status = 400;
        throw new ValidationError(INVALID_TOKEN_MSG);
      }
...
export const invitationRoutes = new Elysia().use(orgInvitationRoutes).use(acceptInvitationRoute);
