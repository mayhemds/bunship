...
import { invalidateMaintenanceCache } from "../plugins/maintenanceMode";
...
export async function impersonateUser(
  ...
  await db.insert(auditLogs).values({
    ...
    resourceType: "user",
    resourceId: user.id,
    metadata: { targetEmail: user.email, sessionId: session!.id, expiresAt },
  });
...
export async function listOrganizations(
  ...
  return {
    data: data as (Organization & { subscriptionStatus?: string })[],
    pagination: {
      page,
      limit,
      total: count2,
...
/**
 * Enable or disable maintenance mode. Persisted in the `app_settings` table
 * so it survives restarts and is shared across API instances.
 */
export async function setMaintenanceMode(enabled: boolean): Promise<{ enabled: boolean }> {
  ...
  await db
    .insert(appSettings)
    .values({ key: MAINTENANCE_MODE_KEY, value: { enabled }, updatedAt: now })
    .onConflictDoUpdate({
...
/**
 * Read maintenance mode. Falls back to the MAINTENANCE_MODE env var when no
 * persisted row exists (useful for fresh installs and emergency bootstraps).
 */
export async function getMaintenanceMode(): Promise<{ enabled: boolean }> {
  ...
  if (row && typeof (row.value as { enabled?: unknown }).enabled === "boolean") {
    return { enabled: (row.value as { enabled: boolean }).enabled };
  }

  return { enabled: process.env.MAINTENANCE_MODE === "true" };
}
