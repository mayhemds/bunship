...
export async function login(input: LoginInput): Promise<LoginResult> {
  ...
  if (user.twoFactorEnabled) {
    ...
    const isValidTOTP = await verifyTOTP(decryptedSecret, twoFactorCode);

    if (!isValidTOTP) {
      // Fall back to backup code verification (one-time use)
      const isValidBackup = await consumeBackupCode(user.id, twoFactorCode);
      if (!isValidBackup) {
...
/**
 * Generate QR code data URL for TOTP (PNG image as data: URL)
 */
async function generateQRCode(email: string, secret: string): Promise<string> {
  ...
  return QRCode.toDataURL(otpauthUrl, { errorCorrectionLevel: "M", width: 256, margin: 1 });
}

/**
 * Verify and consume a one-time backup code. Returns true and deletes the
 * matching code row if valid; returns false otherwise.
 */
async function consumeBackupCode(userId: string, code: string): Promise<boolean> {
  ...
  const codeHash = await hmacHash(process.env.JWT_SECRET!, normalized);

  const deleted = await db
    .delete(backupCodes)
    .where(and(eq(backupCodes.userId, userId), eq(backupCodes.codeHash, codeHash)))
    .returning();
