...
export async function getUsage(orgId: string) {
  ...
  // Cast limits to concrete type for runtime -1 check (unlimited plans use -1)
  ...
  const [apiRequestRows, memberRows, projectRows, webhookRows, apiKeyRows] = await Promise.all([
    db
  ...
  const apiRequestCount = apiRequestRows[0]?.apiRequestCount ?? 0;
  const memberCount = memberRows[0]?.memberCount ?? 0;
  const projectCount = projectRows[0]?.projectCount ?? 0;
  const webhookCount = webhookRows[0]?.webhookCount ?? 0;
  const apiKeyCount = apiKeyRows[0]?.apiKeyCount ?? 0;
  ...
  return {
    ...
    usage: {
      members: {
        ...
        percentage: limits.members === -1 ? 0 : (memberCount / limits.members) * 100,
      },
      projects: {
      ...
      apiRequests: {
        ...
        percentage: limits.apiRequests === -1 ? 0 : (apiRequestCount / limits.apiRequests) * 100,
      },
      webhooks: {
        current: webhookCount,
        limit: limits.webhookEndpoints,
        percentage:
          limits.webhookEndpoints === -1 ? 0 : (webhookCount / limits.webhookEndpoints) * 100,
      },
      apiKeys: {
        current: apiKeyCount,
        limit: limits.apiKeys,
        percentage: limits.apiKeys === -1 ? 0 : (apiKeyCount / limits.apiKeys) * 100,
      },
    },
  };
}
